...
import numpy as np
import random
from data.inputs.utilities import ally_locs, enemy_locs, nearest_enemy_selective, getMovement, getDirection, getDistance, tagConverter, myStar
...
class IndependentLearnerAll(MultiAgentEnv):
    def __init__(self, args, agents, team=0, mapChange=True):
        ...
        # wait for the first state to get this info
        self.truckID =0
        self.tanklID=0
        self.tankhID=0
        self.droneID=0

        # handle this in late_init
    ...
    def late_init(self, obs):
        ...
        self.init_agents = copy.copy(self.agents)
        ...
        for i in range(len(self.agents)):
            self.agents_positions[self.agents[i]]=my_units[i]['location']

        self.init_agents_positions = copy.copy(self.agents_positions)
    ...
    def reset(self):
        ...
        self.agents_positions = copy.copy(self.init_agents_positions)

        # TODO: set self.agents_position_init in late_unit
        # for i in range(len(self.agents)):
        #     self.agents_positions[self.agents[i]]=(self.configs['blue']['units'][i]['y'], self.configs['blue']['units'][i]['x'])
        
        # set init_init_stuff in late init
    ...
    def terrain_locs(self, terr):
        ...
        y_max, x_max = terr.shape
        ter_locs = {}
        for i in range(y_max):
            for j in range(x_max):
               if terr[i][j] != 0:
                    ter_locs[i*self.width+j]= terr[i][j]
        return ter_locs
